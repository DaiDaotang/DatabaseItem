...
public class CheckResultDao {
    ...
    public List<ResultBean> GetResultByTeam(){
        ...
        try{
            conn = DBUtil.getConnection();
            conn.setAutoCommit(false);
            pStatement = conn.prepareStatement("select ath_id,athname,result,finals,rank,team_id from competition natural join participation natural join athletes natural join m_item where team_id in(select team_id from team where team_name = ?)");
